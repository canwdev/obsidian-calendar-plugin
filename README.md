...
- 生成的文件会放在 `dist/.obsidian/plugins/calendar-react` 目录下，可以用 Obsidian 直接打开 dist 仓库。
- 也可以创建 `.env` 文件并指定构建输出目录（如 `OUT_DIR=../TestPlugins/.obsidian/plugins/calendar-react`）。
